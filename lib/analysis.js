...
export function analyzeControl(name, genomeName, controlText) {
  return new Promise(function(resolve, reject) {
    ...
    resolve({ name: name, genomeName: genomeName, heightMappings: heightMappings, rawData: igv.rawData, stats: igv.stats, type: "control"});
  });
}
...
export function analyzeExperiment(name, analyzedGenome, analyzedControl, experimentText) {
  ...
  var heightMappings = deriveHeightListing(igv.rawData);
  return deriveFeatures({ rawData: analyzedControl.rawData, stats: analyzedControl.stats }, igv, analyzedGenome.map).then(function(features) {
