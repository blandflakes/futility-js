...
function deriveHeightListing(rawIgvData) {
  var canCoalesce = function(left, right) {
    return left.end === right.start && left.reads === right.reads;
  }

  var mappings = [];
  rawIgvData.forEach(function(row) {
    var readPosition = { start: row[1],
                          end: row[2],
                          reads: row[3]
                        };
    if (readPosition.reads === 0) { return; }
    var toInject = readPosition;
    if (mappings.length > 0) {
      var previousMapping = mappings[mappings.length - 1];
      if (canCoalesce(previousMapping, readPosition)) {
        previousMapping.end = readPosition.end;
        toInject = previousMapping;
      }
    }
    injectReferences(mappings, toInject, toInject.start, toInject.end);
  });
  return mappings;
}
