...
function deriveHeightListing(rawIgvData, taggers) {
  ...
  var redThreshold = atPercentile(0.99999, distinctMappings, function(first, second) { return first.reads - second.reads }).reads;
...
export function analyzeControl(name, genomeName, controlText) {
  return new Promise(function(resolve, reject) {
    ...
    resolve({ name: name, genomeName: genomeName, heightMappings: heightMappings, backingIgv: igv, type: "control"});
  });
}
...
export function analyzeExperiment(name, analyzedGenome, analyzedControl, experimentText) {
  ...
  return deriveFeatures(analyzedControl.backingIgv, igv, analyzedGenome.map).then(function(features) {
    return { name: name, controlName: analyzedControl.name, genomeName: analyzedGenome.name, heightMappings: heightMappings, backingIgv: igv, features: features, type: "experiment"};
  });
}
